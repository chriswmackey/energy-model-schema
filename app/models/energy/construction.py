...
class EnergyMaterial(BaseModel):
    """Material "\types" used to describe layers within opaque construction elements."""
    ...
    visible_absorptance: float = Schema(
        ...
        ge=0,
        le=1,
        description='Fraction of incident visisible wavelength radiation absorbed by the \
...
class EnergyWindowMaterialAirGap(BaseModel):
    """Used to create single layer of air gap which can be combined with
  EnergyWindowMaterialGlazing to make multi-pane windows."""
    ...
    conductivity_coeff_B: float = Schema(
        ...,
        description='The A coefficient for gas conductivity in W/(m-K2). Used only if Gas \
          Type is Custom.'
    )

    conductivity_coeff_C: float = Schema(
        ...,
        description='The A coefficient for gas conductivity in W/(m-K3). Used only if Gas \
          Type is Custom.'
    )
    ...
    viscosity_coeff_C: float = Schema(
        ...,
        description='The C coefficient for gas viscosity in kg/(m-s-K2). Used only if Gas \
          Type is Custom.'
    )
    ...
    specific_heat_coeff_A: float = Schema(
        ...,
        gt=0,
        description='The A coefficient for gas specific heat in J/(kg-K). Used only if Gas \
          Type is Custom.'
    )
    ...
    specific_heat_coeff_B: float = Schema(
        ...,
        gt=0,
        description='The B coefficient for gas specific heat in J/(kg-K2). Used only if Gas \
          Type is Custom.'
    )
...
class EnergyWindowMaterialSimpleGlazSys(BaseModel):
    """Used to describe an entire glazing sytem rather than individual layers. Used when 
    only very limited information is available on the glazing layers or when specific 
    performance levels are being targeted."""
    ...
    u_factor: float = Schema(
        ...
        gt=0,
        le=5.8,
        description='Used to descibe the value for window system U-Factor, or overall \
    ...
    SHGC: float = Schema(
        ...
        gt=0,
        lt=1,
        description='Unitless  quantity describing Solar Heat Gain Coefficient for normal \
          incidence and vertical orientation.'
    )
...
class EnergyWindowMaterialBlind(BaseModel):
    """Used to specify the properties of a window blind consisting of flat, equally-spaced slats."""
    ...
    slat_separation: float = Schema(
        ...
        gt=0,
        le=1,
        description='The distance between the front of a slat and the back of the adjacent\
             slat in meters.'
    )
    ...
    beam_solar_transmittance: float = Schema(
        ...
        ge=0,
        lt=1,
        decription='The beam solar transmittance of the slat, assumed to be independent \
    ...
    front_diffuse_solar_reflectance: float = Schema(
        ...
        ge=0,
        lt=1,
        description='The front-side slat reflectance for hemispherically diffuse solar radiation.\
    ...
    back_diffuse_solar_reflectance: float = Schema(
        ...
        ge=0,
        lt=1,
        description='The back-side slat reflectance for hemispherically diffuse solar radiation. \
            Default value is 0.5.'
    )
    ...
    diffuse_visible_transmittance: float = Schema(
        ...
        ge=0,
        le=1,
        description='The slat transmittance for hemispherically diffuse visible radiation.\
             This value should equal “Slat Beam Visible Transmittance.”'
    )
...
class OpticalDatatype (str, Enum):
    ...
    description = 'For SpectralAverage optical data type, solar transmittance and \
...
class SolarDiffusing (str, Enum):
    no = 'No'
    yes = 'Yes'
    description = 'Takes values No and Yes. If No (default), the beam solar radiation \
...
class EnergyWindowMaterialGlazing(BaseModel):
    """Used to describe a single glass pane corresponding to a layer in a window construction."""
    ...
    solar_transmittance: float = Schema(
        ...
        ge=0,
        le=1,
        description='Transmittance of solar radiation through the glass at normal incidence.\
             Default value is 0.'
    )
    ...
    visible_reflectance_back: float = Schema(
        ...
        description='Reflectance of visible light off of the back side of the glass at \
          normal incidence averaged over the solar spectrum and weighted by the response of the human eye.'
    )
    ...
    front_emissivity: float = Schema(
        ...
        description='Infrared hemispherical emissivity of the front (outward facing) \
          side of the glass.  Default value is 0.84, which is typical for clear glass without \
            a low-e coating.'
    )
...
class EnergyWindowMaterialShade (BaseModel):
    ...
    top_opening_multiplier: float = Schema(
        ...
        ge=0,
        le=1,
        decription='The effective area for air flow at the top of the shade, divided by \
    ...
    airflow_permeability: float = Schema(
        ...
        description='The fraction of the shade surface that is open to air flow. \
            If air cannot pass through the shade material, airflow_permeability = 0. Default \
                value is 0.'
    )
...
class EnergyConstruction(BaseModel):
  ...
  the building envelope and interior elements that is the walls, roofs, floors, windows and
  doors of the building.
    ...
    materials: Union[EnergyMaterial, EnergyMaterialNoMass, EnergyWindowMaterialAirGap,
                     EnergyWindowMaterialSimpleGlazSys, EnergyWindowMaterialBlind, EnergyWindowMaterialGlazing,
