from app.models.energy.ScheduleRuleset import DateTime, ScheduleContinuous, ScheduleDiscrete, NumericType, UnitType, ScheduleTypeLimits, YesOrNo, ScheduleDay, ScheduleRuleset, ScheduleRule
...
def test_schedule_rule_wrong(): 
    ...
    with pytest.raises(ValidationError): ScheduleRuleset.parse_obj(wrong_schedule_type_limits) 
    wrong_lower_limit = copy(schedule_ruleset_1)
    wrong_lower_limit['lower_limit_value'] = -1
    with pytest.raises(ValidationError): ScheduleRuleset.parse_obj(wrong_lower_limit)
    wrong_default_day = copy(schedule_ruleset_1)
    wrong_default_day['default_day_schedule'][0]['hour'] = 25
    with pytest.raises(ValidationError): ScheduleRuleset.parse_obj(wrong_default_day)
    wrong_default_day_minute = copy(schedule_ruleset_1)
    wrong_default_day_minute['default_day_schedule'][2]['minute'] = 60
    with pytest.raises(ValidationError): ScheduleRuleset.parse_obj(wrong_default_day_minute)
    wrong_schedule_rule = copy(schedule_ruleset_1)
    wrong_schedule_rule['schedule_rule'][0]['start_period']['month'] = 24
    with pytest.raises(ValidationError): ScheduleRuleset.parse_obj(wrong_schedule_rule)
    wrong_leap_year = copy(schedule_ruleset_1)
    wrong_leap_year['schedule_rule'][0]['start_period']['is_leap_year'] = 'Yes'
    with pytest.raises(ValidationError): ScheduleRuleset.parse_obj(wrong_leap_year)
...
def test_schedule_file_wrong():
    ...
    wrong_values['values'] = [] 
    with pytest.raises(ValidationError): ScheduleFile.parse_obj(wrong_values)
    wrong_start = copy(schedule_file)
    wrong_start['start_day_of_year'] = 367
    with pytest.raises(ValidationError): ScheduleFile.parse_obj(wrong_start)
    wrong_unit = copy(schedule_file)
    wrong_unit['units'] = 'Nil'
    with pytest.raises(ValidationError): ScheduleFile.parse_obj(wrong_unit)
